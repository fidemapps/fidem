...
if (process.env.NODE_ENV === 'browser') {
  ...
  methods = {
    assignMemberToSession: require('./methods/assignMemberToSession'),
    logAction: require('./methods/logAction'),
    startSession: require('./methods/startSession')
  };
} else {
  request = require('request');
  methods = {
    assignMemberToSession: require('./methods/assignMemberToSession'),
    authenticate: require('./methods/authenticate'),
    createMember: require('./methods/createMember'),
    getUserByToken: require('./methods/getUserByToken'),
    logAction: require('./methods/logAction'),
    startSession: require('./methods/startSession')
  };
}
...
Client.prototype.request = function (options, cb) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    options = assign({
      token: null,
      sign: process.env.NODE_ENV !== 'browser',
      method: 'GET',
      body: null,
      headers: {},
      requestOptions: {}
    ...
    // Sign the request.
    if (process.env.NODE_ENV !== 'browser')
      require('./sign')(options, client, req);
