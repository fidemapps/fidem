 ...
 * @param {string} [config.hostname="api.fidem360.com"] Hostname.
...
function Client(config) {
    ...
    this.config = _.defaults(config, {
        ...
        port: config.protocol === 'http' ? 80 : 443,
        protocol: 'https'
    });
}
 ...
 * @param {string} [options.bearerToken=null] Bearer access token. If provided,
...
/**
 * Retrieve a user by bearer token.
 *
 * @param {string} token Token of the user.
 * @param {Function} [cb] Optional callback.
 * @returns {Promise}
...
Client.prototype.getUserByBearerToken = function (bearerToken, cb) {
    return this.request({
        sign: false,
        bearerToken: bearerToken,
        path: '/internal/security/user/me'
    }, cb);
};
